            ...
            <form action="{{ route('company.jobs.update', $job->id) }}" method="POST" class="space-y-6">
                ...
                <div class="space-y-4">
                    <label class="block text-sm font-medium text-slate-700">
                        ...
                        <input type="text" name="title" value="{{ old('title', $job->title) }}" required class="mt-1.5 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm text-slate-900 outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
                    ...
                    <div class="grid gap-4 md:grid-cols-2">
                        <label class="block text-sm font-medium text-slate-700">
                            ...
                            <input type="text" name="location" value="{{ old('location', $job->location) }}" required class="mt-1.5 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm text-slate-900 outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
                    ...
                    <div class="grid gap-4 md:grid-cols-3">
                        <label class="block text-sm font-medium text-slate-700">
                            ...
                            <input type="text" name="salary" value="{{ old('salary', $job->salary) }}" class="mt-1.5 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm text-slate-900 outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
                            @error('salary') <p class="mt-1 text-xs text-rose-500">{{ $message }}</p> @enderror
                        </label>

                        <label class="block text-sm font-medium text-slate-700">
                            Batas Lamaran (Deadline)
                            <input type="date" name="deadline" value="{{ old('deadline', optional($job->deadline)->format('Y-m-d')) }}" class="mt-1.5 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm text-slate-900 outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
                    ...
                    <label class="block text-sm font-medium text-slate-700">
                        ...
                        <textarea name="description" rows="4" required class="mt-1.5 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">{{ old('description', $job->description) }}</textarea>
                        @error('description') <p class="mt-1 text-xs text-rose-500">{{ $message }}</p> @enderror
                    </label>

                    <label class="block text-sm font-medium text-slate-700">
                        Persyaratan (Requirements)
                        <textarea name="requirements" rows="4" class="mt-1.5 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">{{ old('requirements', $job->requirements) }}</textarea>
                        @error('requirements') <p class="mt-1 text-xs text-rose-500">{{ $message }}</p> @enderror
                    </label>

                    <label class="block text-sm font-medium text-slate-700">
                        Tanggung Jawab (Responsibilities)
                        <textarea name="responsibilities" rows="4" class="mt-1.5 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">{{ old('responsibilities', $job->responsibilities) }}</textarea>
